...
use entity::sys_user::{ActiveModel, Model as UserModel};
...
impl UserSchema {
    ...
    pub fn try_from_active_model(user: ActiveModel) -> ApiResult<Self> {
        ...
        auth_utils::generate_token(user.id.unwrap(), user.created_at.unwrap())
            .map(|token| Self::new(user.username.unwrap(), token))
    }

    /// Create a user schema from a user model, will generate a token
    pub fn try_from_model(user: UserModel) -> ApiResult<Self> {
        auth_utils::generate_token(user.id, user.created_at)
            .map(|token| Self::new(user.username, token))
    }
