...
use entity::sys_user::{Column as UserColumn, Entity as User, Model as UserModel};
use hmac::{Hmac, Mac};
...
pub fn generate_token(user_id: i64, created_date: i64) -> ApiResult<String> {
...
pub async fn get_user_by_token(db: &DatabaseConnection, token: &str) -> ApiResult<UserModel> {
    ...
    if claims.get_created_at() != user.created_at {
...
pub async fn get_user_by_username_and_password(
    ...
    User::find()
        .filter(
            UserColumn::Username
                .eq(username)
                .and(UserColumn::Password.eq(hashed_password)),
        )
        .one(db)
        .await
        .database_err()?
